...
Blizzard.prototype.get = function get (path, args, instance) {
  ...
  const endpoint = endpoints.getEndpoint(config.origin || this.defaults.origin, config.locale || this.defaults.params.locale, config.oauth || false);
  ...
  if (obj.params.access_token) {
    ...
    obj.headers.Authorization = `Bearer ${obj.params.access_token}`;
    delete obj.params.access_token;
  }

  return this.axios.get(`${endpoint.hostname}${encodeURI(path)}`, obj);
};

function getOauthDomain(origin) {
  if (origin === 'cn') {
    return 'oauth.battlenet.com.cn';
  } else {
...
Blizzard.prototype.fetchToken = function fetchToken (origin, params) {
  ...
  return this.axios.post(`https://${getOauthDomain(origin)}/oauth/token?grant_type=${encodeURIComponent(grant_type)}&client_id=${encodeURIComponent(client_id)}&client_secret=${encodeURIComponent(client_secret)}`);
};
...
Blizzard.prototype.checkToken = function checkToken (origin, params) {
  return this.axios.get(`https://${getOauthDomain(origin)}/oauth/check_token`, params);
};
